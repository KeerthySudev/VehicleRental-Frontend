...
const BookingsPage = () => {
  ...
  const [rentOutVehicle] = useMutation(RENT_OUT_VEHICLE);
  ...
  const handleRent = async (id: any) => {
    ...
    toast.success("Status updated!", { position: "top-right", autoClose: 2000 });
  };
        ...
        <div className={styles.bookingCardContainer}>
          {bookings.map((booking : any) => (
            ...
            <div className={styles.bookingPickup}>
                ...
                <p className={styles.bookingLocation}>{booking.customer.name}</p>
              </div>
  ...
  <button onClick={() => handleRent(booking.id)} className={styles.rentButton}>Rent Out</button>
) : booking.status === 'Rented' ? (
  <button onClick={() => handleRent(booking.id)} className={styles.rentButton}>Returned</button>
) : (
  ""
)}
